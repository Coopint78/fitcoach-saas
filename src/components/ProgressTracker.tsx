...
export default function ProgressTracker({ clientId }: Props) {
  ...
  async function handleSave() {
    ...
    if (!hasData && !photoFile) { toast.error("Ingresá al menos una medida o foto"); return; }
    setSaving(true);
    ...
    if (photoFile) {
      ...
      if (!res.ok) { toast.error(data.error ?? "Error al subir foto"); setSaving(false); return; }
      photoUrl = data.url;
    }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold flex items-center gap-2 text-base">
          <TrendingUp className="h-5 w-5 text-indigo-600" /> Progreso
        </h2>
      ...
      {metrics.length > 0 && (
        <div className="grid md:grid-cols-2 gap-4">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Peso (kg)</CardTitle></CardHeader>
            <CardContent><ProgressChart metrics={metrics} metric="weight_kg" label="Peso" color="#6366f1" unit="kg" /></CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Cintura (cm)</CardTitle></CardHeader>
            <CardContent><ProgressChart metrics={metrics} metric="waist_cm" label="Cintura" color="#f59e0b" unit="cm" /></CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Caderas (cm)</CardTitle></CardHeader>
            <CardContent><ProgressChart metrics={metrics} metric="hips_cm" label="Caderas" color="#ec4899" unit="cm" /></CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Pecho (cm)</CardTitle></CardHeader>
            <CardContent><ProgressChart metrics={metrics} metric="chest_cm" label="Pecho" color="#10b981" unit="cm" /></CardContent>
          </Card>
        </div>
      )}
      ...
      {photos.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2"><Camera className="h-4 w-4 text-indigo-600" /> Fotos de progreso</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {metrics.length === 0 && (
        <div className="text-center py-10 border-2 border-dashed border-border rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground">Sin registros de progreso todavía</p>
        </div>
      )}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="rounded-2xl max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <Label>Notas</Label>
              <Input value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Cómo me sentí hoy…" className="rounded-xl h-10" />
            </div>
            ...
            <div className="space-y-1">
              ...
              {photoPreview ? (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">Tocá para agregar foto</p>
                </div>
              )}
