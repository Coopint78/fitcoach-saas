...
export async function POST(request: NextRequest) {
  // Security: Only allow from admin/internal requests
  ...
  if (!trainer.confirm_token) {
    return NextResponse.json({ error: "No confirmation token found - trainer may already be confirmed" }, { status: 400 });
  }
  ...
  const trainerName = name || trainer.name || email.split("@")[0];
  ...
  try {
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px">
          <h2 style="color:#111827;margin-top:0">Bienvenido/a a FitCoach, ${trainerName.split(" ")[0]}!</h2>
    ...
    return NextResponse.json({ ok: true, message: `Confirmation email resent to ${trainer.email}` });
  } catch (err) {
